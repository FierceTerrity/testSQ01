...
function createUserIfNullableOrUpdateName(steamID, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (!resultMain) {
                //console.log(`add user ${fields.name} in main`)
                yield collectionMain.insertOne(fields);
            }
            if (!resultTemp) {
                //console.log(`add user ${fields.name} in temp`)
...
function updateUser(steamID, field, weapon) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        //console.log(`update user ${steamID}`)
        yield collectionMain.updateOne(user, doc);
        yield collectionTemp.updateOne(user, doc);
...
const chatCommands = (state, options) => {
    ...
    const { adminsEnable, reportEnable, stvolEnable, fixEnable, discordEnable, statsEnable, bonusEnable, swapEnable, swapTimeout, statsTimeout, stvolTimeout, } = options;
    let players = [];
    let timeoutPlayers = [];
    const swapHistory = [];
    const admins = (data) => {
        if (!adminsEnable)
            return;
        adminWarn(execute, data.steamID, '-✯- На сервере присутствует администрация -✯-');
        adminWarn(execute, data.steamID, '-✯- Для связи с администрацией/модераторами создайте тикет в дискорде  discord.gg/bestinhell -✯-');
    };
    const report = (data) => {
        if (!reportEnable)
            return;
        adminWarn(execute, data.steamID, `-✯- Если вы зафиксировали грубое нарушение -> Пожалуйста создайте тикет в нашем дискорде discord.gg/bestinhell -✯-`);
    };
    const stvol = (data) => {
        if (!stvolEnable)
            return;
        ...
        adminBroadcast(execute, `${name} выбрасывает число  ${range}`);
    ...
    const discord = (data) => {
        if (!discordEnable)
            return;
        adminWarn(execute, data.steamID, '-✯- Discord сервера - discord.gg/bestinhell -✯-');
        adminWarn(execute, data.steamID, '-✯- Либо в дискорде Добавить сервер -> Лучшие в Аду -✯-');
    };
    ...
    const bonus = (data) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        adminWarn(execute, steamID, 'Для получения вип-статуса      используйте !vip в нашем дискорде                 -✯-  discord.gg/bestinhell -✯-');
    ...
    const leaderChanged = (data) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (isLeader) {
            if (!leaderRole) {
                ...
                timer = setInterval(() => __awaiter(void 0, void 0, void 0, function* () {
                    var _a, _b;
                    let updatedPlayer = (_a = state.players) === null || _a === void 0 ? void 0 : _a.find((user) => user.steamID === player.steamID);
                    ...
                    if (seconds <= 0) {
                        ...
                        updatedPlayer = (_b = state.players) === null || _b === void 0 ? void 0 : _b.find((user) => user.steamID === player.steamID);
...
const initSquadJS = (_a) => __awaiter(void 0, [_a], void 0, function* ({ id, mapsName, mapsRegExp, plugins, rcon, logs, }) {
    ...
    if (configs === null || configs === void 0 ? void 0 : configs.length) {
        for (const config of configs) {
            try {
                ...
                //console.log(rcon,logs,config.id)
                yield initSquadJS({
                    rcon,
                    logs,
                    id: config.id,
